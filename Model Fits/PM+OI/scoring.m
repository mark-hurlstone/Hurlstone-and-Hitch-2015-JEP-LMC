...
score.accspc = sum(acctmp)./parms.nTrials; 

% RT SPC
allrtstmp = allRts;
incorrect = acctmp == 0;
allrtstmp(incorrect) = 0; 
score.crtspc = sum(allrtstmp)./sum(acctmp);
score.rtspc = nansum(allRts)./parms.nTrials;
...
score.transrt = sum(transrt)./score.transreps;
...
score.fltrdtransrt = sum(transrt)./score.transreps;

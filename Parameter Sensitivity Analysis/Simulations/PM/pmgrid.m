...
nsims = length(pspace.ItemWeight)*length(pspace.ItemDistinct);
...
grid.state = zeros(nsims, 2);                  % Holds parameter vectors
...
for ItemWeight = pspace.ItemWeight
	for ItemDistinct = pspace.ItemDistinct
        ...
        grid.state(pvec,:) = [ItemWeight,ItemDistinct];
    end
    pvec
end

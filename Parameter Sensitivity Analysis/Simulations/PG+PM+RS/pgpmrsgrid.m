...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                 %
% GRID SEARCH FOR PG+PM+RS MODEL  %
%                                 %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% PRIMACY GRADIENT 
parms.GradStart           = 1;     % Start value for primacy gradient
...
nsims = length(pspace.ItemDistinct)*length(pspace.Mix) ...
*length(pspace.GradDecrease)*length(pspace.ResSupp);
...
grid.state = zeros(nsims, 4);                   % Holds parameter vectors
	...
	for Mix = pspace.Mix
        for GradDecrease = pspace.GradDecrease
            for ResSupp = pspace.ResSupp
                ...
                parms.Mix          = Mix;
                parms.GradDecrease = GradDecrease;
                ...
                grid.state(pvec,:)        = [ItemDistinct,Mix,GradDecrease,ResSupp];
            end
        end
    end
    pvec
